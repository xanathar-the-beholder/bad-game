...
public class M extends KeyAdapter  {
	...
	private M() throws Exception {
		...
		do {
			...
			//
			//
			generateMaze(maze, todo, gt, gv, gu, gbnd, gx, gy, mazex, mazey);
			//
			// End of maze generation
			//
			placeItems(maze, gt, gv, gp, gu, gbnd, gx, gy);
			//
			//
			while (!restart) {
				//
				...
				for (int gobj=0;gobj<255;gobj++) {
					...
					bullet = updateGameObject(gt, gv, ga, gp, gu, gx, gy, gdx, gdy, binh, gobj, dx, dy, a, bullet);			
					//
					// Collision detection (with maze).
					//
					handleMazeCollisions(maze, gt, ga, gu, gx, gy, gdx, gdy, gobj, nx, ny);
					//
					...
					//
					a = drawGame(invincible, gv, ga, gp, toggle, gu, gblk, gx, gy, gdx, gdy, g, gobj, a);
				}
				//
				g.translate(gx[0],gy[0]);
	...
	private double drawGame(int invincible, int[] gv, int[] ga, int[] gp, boolean toggle, boolean[] gu,
	...
	private void handleMazeCollisions(int[][] maze, int[] gt, int[] ga, boolean[] gu, double[] gx, double[] gy, double[] gdx,
	...
	private boolean updateGameObject(int[] gt, int[] gv, int[] ga, int[] gp, boolean[] gu, double[] gx, double[] gy,
	...
	private void placeItems(int[][] maze, int[] gt, int[] gv, int[] gp, boolean[] gu, double[] gbnd, double[] gx,
			double[] gy) {
		int citem=30;
		for (t=0;t<4;t++) {
			while(true) {
	...
	private void generateMaze(int[][] maze, int[] todo, int[] gt, int[] gv, boolean[] gu, double[] gbnd, double[] gx,
